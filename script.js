    ...
    icon.addEventListener("click", (element) => {
        if(element.target.classList.contains("fa-copy")){
            if(element.target.id == "from") {
                navigator.clipboard.writeText(fromText.value); //Kopyalama işleminin gerçekleştiği yer
            }else {
                navigator.clipboard.writeText(toText.value); //Kopyalama işleminin gerçekleştiği yer
            }
        }else {
            let utterance;
            if(element.target.id == "from") {
